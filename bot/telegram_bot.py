...
class TelegramBot:
    ...
    async def _handle_ai_callback(
        ...
        await self._run_ai_turn(
            ...
            exchange=exchange,
            user_text=(
                "Разбор 1–3ч. Смотри вперёд: куда цена пойдёт дальше, не только «ждать». "
                "Формат: ВЕРДИКТ → КУДА ЖДЁМ (базовый путь по уровням) → ЧТО ДЕЛАТЬ (триггер/лимит, стоп, TP1/TP2). "
                "TP только с уровней пакета/магнитов/swing, не проценты. Без markdown. "
                "Окно 24h. Если heatmap недоступен — LIQ_MAGNET."
            ),
            hours=24,
